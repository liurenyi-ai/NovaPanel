...
docker run -d \
  ...
  --name 3x-ui \
  ghcr.io/ruofei-nova/novapanel:latest
```
...
[صفحهٔ releases](https://github.com/ruofei-nova/NovaPanel/releases) دانلود کنید، آن را
...
```bash
XUI_NONINTERACTIVE=1 bash <(curl -Ls https://raw.githubusercontent.com/ruofei-nova/NovaPanel/main/install.sh)
```

دایرکتوری [`deploy/`](https://github.com/ruofei-nova/NovaPanel/tree/main/deploy) در مخزن
